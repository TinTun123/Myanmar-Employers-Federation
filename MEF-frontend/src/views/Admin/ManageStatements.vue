<template>
  <AdminTitleComponent :title="'Manage Statements'"></AdminTitleComponent>
  ...
  <div class="flex flex-col mx-6 my-6 gap-4 md:my-6 md:mx-6 md:grid md:grid-cols-12">
    <StatementCard class="md:col-span-5 lg:col-span-4" :class="[index % 2 === 0 || index === 0 ? 'md:col-start-2' : '']"
      v-for="(statement, index) in statements" :key="index" :statement="statement"></StatementCard>
  </div>
...
const statementStore = useNewsStore()
function goTo(name) {
...
function fetchStatements() {
  if (statementStore.statements?.data?.length) {
    statements.value = statementStore.statements.data;
    return;
  } else {
    statementStore.fetchStatements().then(() => {
      statements.value = statementStore.statements.data;
    });
  }
}
