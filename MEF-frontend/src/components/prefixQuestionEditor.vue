<template>
  <div class="flex flex-col gap-4 md:w-[80%]">
    ...
    <div class="flex flex-col gap-4 md:w-[80%]">
      ...
      <div v-if="!model.options.length" class="text-center py-4">
        <span>No options added yet!</span>
      </div>

      <div v-for="(option, index) in model.options" :key="index" class="flex items-center gap-4">
        <div class="flex-1">
          <input
            v-model="option.short_form"
            @input="updateModel"
            type="text"
...
function removeOption(index) {
  props.model.options.splice(index, 1)
  updateModel()
}
</script>
