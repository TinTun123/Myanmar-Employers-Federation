<template>
  <AdminTitleComponent
    :title="route.params.id ? 'Edit Statement' : 'Create Statement'"
  ></AdminTitleComponent>
  ...
  <div class="mt-6 mx-6 md:mx-[48px] flex flex-col gap-4">
    ...
    <div class="flex justify-end items-center gap-4 text-sm mx-4 my-4">
      <div
        class="bg-white border border-black/40 shadow-lg rounded-full px-4 py-2 flex items-center gap-1"
      >
        <button>Cancel</button>
      </div>
      <div
        class="bg-light-blue text-white font-semibold border border-light-blue shadow-lg rounded-full px-4 py-2 flex items-center gap-1"
      >
        <button>Save</button>
      </div>
    </div>
